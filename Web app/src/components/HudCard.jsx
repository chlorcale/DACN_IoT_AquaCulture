export default function HudCard({ stat }) {
  ...
  const trendArrow = stat.trend === "up" ? "▲" : "▼";
  const rangeClass =
    stat.rangeStatus === "good" ? "status-good" : stat.rangeStatus === "warn" ? "status-warn" : "status-bad";
  ...
  return (
    <div className="hud-card pixel-corners-sm">
      ...
      <div className="bar-track">
        <div className="bar-fill" style={{ width: stat.percent + "%", background: stat.color }} />
      </div>
      <div className="hud-range">
        <span>0</span>
        <span className={rangeClass}>{stat.range}</span>
        <span>10</span>
      </div>
    </div>
  );
